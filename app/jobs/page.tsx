...
export default function JobsPage() {
  ...
  const filePathToApiUrl = (filePath: string, type: "html" | "pdf"): string => {
    if (!filePath) return "";

    // Remove the /shared/reporting-framework/reports/ prefix
    const relativePath = filePath.replace(
      /^\/shared\/reporting-framework\/reports\//,
      ""
    );

    return `/api/reports/${type}/${relativePath}`;
  };
                  ...
                  >
                    ...
                    <div className="flex gap-2 pt-4 border-t">
                      {job.status === "completed" && job.outputs ? (
                        <>
                          {/* Completed: Show report access buttons */}
                          {job.outputs.html && (
                            <Button size="sm" className="flex-1" asChild>
                              <Link
                                href={filePathToApiUrl(
                                  job.outputs.html,
                                  "html"
                                )}
                                target="_blank"
                        ...
                        <>
                          {/* Failed: Show retry and view error buttons */}
                          <Button
                            size="sm"
                            variant="outline"
                            className="flex-1"
                            asChild
                          >
                            <Link href={`/generate/${job.pluginId}`}>
                          ...
                          {job.error && (
                            <Button
                              ...
                              variant="ghost"
                              className="flex-1"
                              onClick={() => handleViewError(job.error!)}
                            >
                        ...
                        <>
                          {/* Queued/Processing: Show refresh status button */}
                          <Button
                            size="sm"
                            variant="outline"
                            className="flex-1"
